...
class Supermarket:
    ...
    def handle_boss_question_about_sara(self):
        sara_purchases = []
        for customer_name, purchased_product, quantity in self.customers:
            if customer_name == "sara":
                sara_purchases.append((purchased_product, quantity))

        if not sara_purchases:
            print("Sara has not bought from the store yet.")
            return

        print("Sarah's shopping list:")
        for purchased_product, quantity in sara_purchases:
            print(f"- {purchased_product}: {quantity} number")
